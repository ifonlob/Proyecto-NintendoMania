...
const interactividadHamburguesa = (cabecera) => {
  const botonHamburguesa = document.querySelector(
    ".cabecera__buscador--hamburguesa",
  );
...
const inicializarDesplegables = () => {
  ...
  const baseDeDatosJuegos = {
    ...
    splatoon: ["Splatoon", "Splatoon 2", "Splatoon 3"],
    kirby: [
      "Kirby's Dream Land",
      "Kirby's Adventure",
      "Kirby Super Star",
...
const inicializarTarjetas = (galeria,imagenesJuegos, coloresSagas) => {
  ...
  const favoritosGuardados =
    JSON.parse(localStorage.getItem("coleccionFavoritos")) || [];
  favoritosGuardados.forEach((favorito) => {
    ...
    h4.classList.add("tarjeta__titulo");
    h4.textContent = juego;
    tarjeta.append(h4);

    const img = document.createElement("img");
    img.src = rutaImagen;
    img.alt = `Imagen de ${juego}`;
    img.classList.add("tarjeta-juego__imagen");
    tarjeta.append(img);

    const parrafoResena = document.createElement("p");
    parrafoResena.classList.add("tarjeta__descripcion");
    ...
    botonDeBorrado.classList.add("tarjeta-juego__boton-borrar");
    botonDeBorrado.textContent = "Eliminar";
    tarjeta.append(botonDeBorrado);

    botonDeBorrado.addEventListener("click", () => {
      const confirmacion = confirm(
        `¿Estás seguro de que quieres eliminar la reseña de ${juego}?`,
      );
      if (confirmacion) {
        tarjeta.remove();
        const favoritosGuardados = JSON.parse(
          localStorage.getItem("coleccionFavoritos"),
        );
        const favoritosFiltrados = favoritosGuardados.filter(
          (favoritoSec) => favoritoSec.id !== id,
        );
  ...
  if (botonBorradoMasivo) {
    botonBorradoMasivo.addEventListener("click", () => {
...
const inicializarAnchorsInicio = () => {
  ...
  const logoSaga = document.querySelector(
    ".cabecera-kirby__menu--imagen, .cabecera-mario__menu--imagen, .cabecera-splatoon__menu--imagen",
  );
...
const inicializarFormularioYValidacion = () => {
  ...
  resenaError.textContent =
    "Error. La reseña tiene que tener una longitud mínima de 15 caracteres.";
  puntuacionError.textContent =
    "Error. La puntuación tiene que estar comprendida entre 0 y 100.";
  ...
  formulario.addEventListener("submit", (evento) => {
    ...
    const selectVacios = [...selectJuegos].filter(
      (select) => select.value.trim() === "",
    );
    const boton = document.querySelector(".gestor-juegos__boton");
    const mensajeError = document.createElement("p");
    if (
      resena.value.trim().length === 0 ||
      selectVacios.length > 0 ||
      puntuacion.value.trim().length === 0
    ) {
      ...
      mensajeError.textContent =
        "⚠️ Por favor, selecciona un juego y rellena todos los campos correctamente antes de publicar.";
    ...
    formulario.reset();
    selectorJuegoSecundario.innerHTML =
      '<option value="" disabled selected>-- Selecciona un juego --</option>';
...
const validacionFormularioContacto = () => {
  ...
  const campoTelefono = document.querySelector(
    ".contacto__formulario--telefono",
  );
  const selectAsunto = document.querySelector(".contacto__formulario--select");
  const campoTextarea = document.querySelector(
  ...
  errorNombre.textContent =
    "Error. El nombre no puede estar formado por menos de 3 caracteres.";
  errorCorreo.textContent =
    "Error. La dirección de correo electrónico tiene que ser válida (Tiene que contener una @ precedida de un punto).";
  errorTelefono.textContent =
    "Error. El teléfono tiene que estar formado por 9 números en formato correcto.";
  errorTextarea.textContent =
    "Error. El mensaje tiene que estar formado por mínimo 30 caracteres.";
  ...
  formularioContacto.addEventListener("submit", (evento) => {
    ...
    const botonBorrado = document.querySelector(
      '.contacto__formulario--boton[type="reset"]',
    );
    const radioSeleccionado = document.querySelector(
      'input[name="saga_favorita"]:checked',
    );
    const politicaPrivacidad = document.querySelector(
      'input[name="acepta_privacidad"]',
    );
    const mensajeErrorFormularioVacio = document.createElement("p");
    mensajeErrorFormularioVacio.classList.add(
      "mensaje-error__formulario-contacto",
    );
    mensajeErrorFormularioVacio.textContent =
      "⚠️ Por favor, rellene todos los campos correctamente antes de publicar.";
    const mensajeExito = document.createElement("p");
    mensajeExito.classList.add("mensaje-exito");
    mensajeExito.textContent =
      "¡Gracias por enviar tu petición! Te contactaremos lo antes posible vía correo electrónico.";

    if (
      selectAsunto.value === "" ||
      !politicaPrivacidad.checked ||
      campoNombre.value === "" ||
      campoCorreo.value === "" ||
      formularioContacto.querySelector(".input-error") ||
      !radioSeleccionado ||
      campoTextarea.value === ""
    ) {
